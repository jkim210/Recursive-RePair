...
int main(int argc, char **argv)
{
  ...
  if (argc != 3)
  {
    fprintf(stderr, "Usage: %s <dictionary filename> <int shift used for pfp++> \n\n", argv[0]);
    exit(1);
  }
  ...
  while (1)
  {
    ...
    if (e != 1)
    {
      ...
      {
        perror("Error reading dictionary word length");
        exit(1);
      }
    }
    // read actual word
    for (int i = 0; i < wlen; i++)
    {
      ...
      if (e != 1)
      {
        perror("Error writing to .int file");
        exit(1);
      }
    }
    count++; //Delete
    ...
    if (++n < 0)
    { // update separator and check it is still valid
