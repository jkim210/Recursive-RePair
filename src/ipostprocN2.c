...
int main(int argc, char **argv)
{
  ...
  while (i < sizeC)
  {
    ...
    {
      if (AdiczC[j] >= terms)
        AdiczC[j] -= (terms - alpha);
      j++;
    }
    assert(AdiczC[j] >= Unique); // this must be a separator
    int ni = j + 1;              // next i
    while (j - i > 1)
    { // balanced grammar construction
      int k = i;
      int ko = i;
      while (k + 1 < j)
      {
        val[0] = AdiczC[k++];
        val[1] = AdiczC[k++]; // new rule to generate AdiczC[k,k+1]
        fwrite(val, sizeof(int), 2, R);
        AdiczC[ko++] = alpha + rules++; // left hand side of the new rule
      }
      if (k < j)
        AdiczC[ko++] = AdiczC[k]; // odd symbol
      j = ko;
    }
  ...
  for (i = 0; i < psizeC; i++)
  {
    ...
    die(e != 1, "Read error");
    if (val[0] < phrases)
      val[0] = transl[val[0]]; // terminal in parse replace with grammar
    else
      val[0] = val[0] - phrases + rules + alpha; // non terminal shifted
